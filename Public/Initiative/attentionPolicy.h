...
// What a game, a video or a presentation looks like: a window covering its whole monitor
...
// Tells a person's input apart from input that never stops. People pause: even steady
// typing leaves a gap of a few seconds every minute or two. A held or stuck key, or a
// controller mapped to one, resets the input clock many times a second for as long as it
// lasts. Observed on the development desktop: Numpad 0 held down, the clock never more
// than 16 ms old, and Revia refusing every chance to speak because the user was
// "mid-keystroke".
class InputRhythm
{
public:
...
private:
    ...
    std::chrono::seconds unbroken;
    std::chrono::steady_clock::time_point lastPause{};
    bool observed = false;
    bool neverPauses = false;
};
...
struct AttentionContext
{
    ...
    // Input has not paused for minutes (see InputRhythm), so sinceLastInput says nothing
    // about whether anyone is typing.
