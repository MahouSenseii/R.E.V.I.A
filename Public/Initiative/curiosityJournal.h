...
public:
    ...
    // The same subject in other words. The planner rephrases rather than repeats:
    // "Digital vs physical permanence" came back as "The paradox of digital permanence
    // vs. physical fragility" four times in ten minutes, and an exact comparison let
    // every one of them through. Topics match when nearly all the meaningful words of
    // the shorter one appear in the other.
    [[nodiscard]] static bool SameTopic(const std::string& left, const std::string& right);
