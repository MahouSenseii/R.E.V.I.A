...
struct Proposal
{
    ...
    std::string message;
    std::string evidence;
    // What it is about, stable while the evidence keeps growing: "25 minutes in Edge"
    // and "40 minutes in Edge" are the same observation. Empty means the evidence is
    // the subject.
    std::string subject;
...
public:
    ...
    // progress is left out, because deciding what to say takes seconds and the moment
    // of speaking checks it again.
    [[nodiscard]] AttentionVerdict SpeakingVerdict(AttentionContext context) const;
