...
struct SelfInquiryResult
{
    ...
    // Her own answer to each question, index-aligned with questions. An entry is empty
    // when she asked but could not answer it yet, which is shown as such, never invented.
    ...
    // The answer she gave herself to question `index`, or empty.
    [[nodiscard]] const std::string& AnswerFor(std::size_t index) const;
    ...
    // Her conclusion alone, restated beside the question it answers, at the end of the
    // prompt. The full block stays in the posture; this is the part the reply has to
    // agree with. Empty when she settled nothing.
    [[nodiscard]] std::string ReplyNote() const;
...
struct SelfInquiryLimits
{
    ...
    // Also think on ordinary questions and tasks, not only on turns the router judged
    // hard. Off here so the library default stays the original behaviour; the settings
    // file turns it on with conversation.selfInquiryScope = "questions".
    bool includeOrdinaryQuestions = false;
    // An ordinary question is shorter than a hard problem statement: "why does my build
    // fail?" is worth working out and is well under the hard-turn threshold.
    std::size_t minimumQuestionCharacters = 16;
