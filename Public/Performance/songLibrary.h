...
// "Revia, sing Bright Lights", "can you sing me something?" -- a request in plain words.
// Only the shape is read here; whether the named song exists is the library's call, and
// a request for one she does not have goes to the model like any other sentence.
struct SingRequest
{
    // The song as the person named it, with the request's wording removed.
    std::string query;
    // "sing something" / "sing a song": any song she has will do.
    bool anySong = false;
};

// Imperative requests only. "I like to sing", "can you sing?" and "singing is fun" are
