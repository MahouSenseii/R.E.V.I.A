...
private:
    ...
    // How Stop reaches the playback thread, guarded by mutex. Stop is called from the
    // speech coordinator, the conversation turn, and /sing stop at once; it signals this
    // copy and never touches `worker`, which only Start and the destructor handle.
    std::stop_source currentStop{std::nostopstate};
    // Serializes Start and the destructor: the two things that join or replace worker.
