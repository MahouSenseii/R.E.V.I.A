...
public:
    ...
    // The constrained shape of one bounded subgoal.
    //
    // Separate from NextStepSchema because it is a different question, and asking it
    // through the other one does not work: the next-step grammar forces an action
    // object, so a model asked for a subgoal returns a step no matter what the prompt
    // says. That is not a wording problem, it is the grammar answering for the model --
    // and it made the assisted path unreachable on a live backend while every scripted
    // test passed.
    //
    // `candidateNames` constrains the target name to something actually on screen when
    // the caller knows what is there. A name the model invents is a name nothing can
    // match, and the run pays a model call to learn that.
    [[nodiscard]] static std::string ComputerSubgoalSchema(
