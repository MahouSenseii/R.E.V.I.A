...
//
// Three different kinds of string live in here and they must not be confused.
//
// `name` is an observed fact: the accessible name the application published. It is
// frequently empty -- a plain Win32 EDIT with nothing beside it has none -- and an empty
// name is not a defect in the application, it is the ordinary case for an input whose
// meaning comes from its position in a form.
//
// `inferredLabel` and `containerName` are *inferences*. The first is what UI Automation
// says labels this element; the second is the nearest ancestor that has a name. Both are
// useful and neither is what the application asserted about this control. They are kept
// in separate fields, rather than filled into `name`, so nothing downstream can mistake
// a guess for a statement -- a policy that matched an inferred label as confidently as a
// published one would be acting on the observer's reasoning while appearing to act on
// the application's.
