...
- ✅ Code review of her own source (`/improve`): evidence-directed and idle reviews, deterministic checks before anything compiles, proof in a private workbench copy with one compiler-guided repair, and accept/reject verdicts fed back into later reviews.
- ⬜ Benchmarking a performance proposal before and after, so "faster" is measured rather than estimated.
