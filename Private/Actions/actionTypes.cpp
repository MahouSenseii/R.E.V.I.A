...
bool RepeatingCouldDuplicateAnEffect(const ActionType value)
{
    switch (value)
    {
        // Observation. Looking twice sees the same thing.
        case ActionType::ListDirectory:
        case ActionType::ReadTextFile:
        case ActionType::InspectWindow:
        case ActionType::WebSearch:
        // Reaching a state rather than adding to one. The window is focused, the
        // directory exists, the pointer is at the point: doing it again is a no-op.
        case ActionType::FocusWindow:
        case ActionType::CreateDirectory:
        case ActionType::MoveCursor:
        // A launch that appeared to work but could not be verified is very often a
        // window that has not finished appearing, and retrying is how that recovers.
        // A second instance is visible and undoable, which is not the class of harm
        // this predicate exists to prevent.
        case ActionType::LaunchApplication:
            return false;
        // Everything else commits something. Typing appends, a chord fires again, a
        // click presses again, a scroll moves further, and the file operations have
