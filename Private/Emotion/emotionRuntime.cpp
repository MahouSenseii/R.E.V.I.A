    ...
    std::vector<RelevantMemory> memories)
{
    ...
    if (!stimulus.description.empty())
    {
        ...
        // Cut on a character boundary: a split one is invalid UTF-8, and a prompt
        // carrying it cannot be encoded, which fails every turn until the cause expires.
        lastCause = revia::utf8::Prefix(stimulus.description, MaximumCauseCharacters);
