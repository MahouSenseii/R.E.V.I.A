...
std::filesystem::path ProgramDirectory(const char* const argumentZero)
{
#ifdef _WIN32
    static_cast<void>(argumentZero);
    std::array<wchar_t, 32768> module{};
    const DWORD length = GetModuleFileNameW(
        nullptr, module.data(), static_cast<DWORD>(module.size()));
    if (length == 0 || length >= module.size())
    ...
    std::error_code error;
    const std::filesystem::path program = std::filesystem::absolute(argumentZero, error);
