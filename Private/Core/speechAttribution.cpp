...
SpeechAttribution ReadSpeechAttribution(const std::string& input)
{
    ...
    // libstdc++ implements repeated regex matches recursively. A long whitespace
    // run can exhaust the native thread stack before prompt fitting ever runs.
    // Collapse whitespace only in the search view and retain original byte offsets;
    // the reported payload and the user's remaining text stay byte-for-byte intact.
    std::string search;
    std::vector<std::size_t> offsets;
    search.reserve(input.size());
    offsets.reserve(input.size() + 1);
    for (std::size_t at = 0; at < input.size(); ++at)
    {
        const bool whitespace = std::isspace(static_cast<unsigned char>(input[at])) != 0;
        if (whitespace && !search.empty() && search.back() == ' ') continue;
        offsets.push_back(at);
        search.push_back(whitespace ? ' ' : input[at]);
    }
    offsets.push_back(input.size());
    static const std::regex report(
        R"(\b((?:someone(?: else)?|somebody(?: else)?|they|he|she|my (?:friend|sister|brother|partner|coworker))\s+(?:said|says|wrote|replied|told (?:me|you|revia))|you\s+(?:said|wrote|replied)|revia\s+(?:said|wrote|replied)|[a-z][a-z'-]{0,63}\s+(?:said|wrote|replied)(?=\s*[:"']))(?:\s+to\s+(you|me|revia))?\s*[: ,]?\s*)",
        std::regex::icase);
    struct Marker { std::size_t begin; std::size_t body; QuotedSpeaker speaker; QuoteRecipient recipient; };
    std::vector<Marker> markers;
    for (std::sregex_iterator it(search.begin(), search.end(), report), end; it != end; ++it)
    {
        const auto position = static_cast<std::size_t>(it->position());
        const auto at = offsets[position];
        ...
        markers.push_back({at, offsets[position + static_cast<std::size_t>(it->length())],
