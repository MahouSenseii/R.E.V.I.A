...
namespace
{
    ...
    void ReadModelTier(const json& data, modelTierSettings& output)
    {
        ...
        // Absent in every settings file written before on-demand residency existed, and
        // absent means off. A setting that changes when a model is present must be
        // asked for, never inherited by upgrading.
        if (data.contains("onDemand"))
            output.bOnDemand = data["onDemand"].get<bool>();
        if (data.contains("idleGraceSeconds"))
            output.idleGraceSeconds = data["idleGraceSeconds"].get<int>();
        if (data.contains("minimumResidencySeconds"))
            output.minimumResidencySeconds = data["minimumResidencySeconds"].get<int>();
    }
...
bool configManager::LoadSettings(appSettings& outSettings) const
{
    ...
    try
    {
        ...
        if (data.contains("llm"))
        {
            ...
            if (llmData.contains("stablePromptPrefix"))
            {
                outSettings.llm.bStablePromptPrefix = llmData["stablePromptPrefix"].get<bool>();
            }
        }
        ...
        if (data.contains("improvement"))
        {
            const json& improvementData = data["improvement"];
            improvementSettings& improvement = outSettings.improvement;
            const auto flag = [&improvementData](const char* key, bool& target)
            {
                if (improvementData.contains(key)) target = improvementData[key].get<bool>();
            };
            const auto number = [&improvementData](const char* key, double& target)
            {
                if (improvementData.contains(key)) target = improvementData[key].get<double>();
            };
            const auto count = [&improvementData](const char* key, int& target)
            {
                if (improvementData.contains(key)) target = improvementData[key].get<int>();
            };
            flag("enabled", improvement.bEnabled);
            flag("explore", improvement.bExplore);
            flag("verify", improvement.bVerify);
            number("minimumBenefit", improvement.minimumBenefit);
            number("evidenceMinimumBenefit", improvement.evidenceMinimumBenefit);
            number("maximumRisk", improvement.maximumRisk);
            count("explorationIntervalMinutes", improvement.explorationIntervalMinutes);
            count("idleMinutesBeforeBuilding", improvement.idleMinutesBeforeBuilding);
            count("maximumAwaitingDecision", improvement.maximumAwaitingDecision);
            count("windowLines", improvement.windowLines);
            count("buildJobs", improvement.buildJobs);
            count("buildTimeoutMinutes", improvement.buildTimeoutMinutes);
            if (improvementData.contains("workbenchPath"))
                improvement.workbenchPath = improvementData["workbenchPath"].get<std::string>();
            if (improvementData.contains("proposalsPath"))
        ...
        if (data.contains("computerControl"))
        {
            const json& controlData = data["computerControl"];
            // Read, never widened. Nothing in this block touches a capability: the mode
            // says who decides the next step, and the recorder says whether anything is
            // kept. An unrecognised mode string is left at its default by
            // ComputerProviderModeFromString rather than being guessed at here.
            if (controlData.contains("providerMode") &&
                controlData["providerMode"].is_string())
            {
                outSettings.computerControl.providerMode =
                    controlData["providerMode"].get<std::string>();
            }
            if (controlData.contains("escalationBudget") &&
                controlData["escalationBudget"].is_number_integer())
            {
                outSettings.computerControl.escalationBudget =
                    controlData["escalationBudget"].get<int>();
            }
            if (controlData.contains("recordingEnabled") &&
                controlData["recordingEnabled"].is_boolean())
            {
                outSettings.computerControl.bRecordingEnabled =
                    controlData["recordingEnabled"].get<bool>();
            }
            if (controlData.contains("datasetDirectory") &&
                controlData["datasetDirectory"].is_string())
            {
                outSettings.computerControl.datasetDirectory =
                    controlData["datasetDirectory"].get<std::string>();
            }
            if (controlData.contains("captureDepth") &&
                controlData["captureDepth"].is_string())
            {
                outSettings.computerControl.captureDepth =
                    controlData["captureDepth"].get<std::string>();
            }
            if (controlData.contains("learnedArtifactPath") &&
                controlData["learnedArtifactPath"].is_string())
            {
                outSettings.computerControl.learnedArtifactPath =
                    controlData["learnedArtifactPath"].get<std::string>();
            }
        }
        ...
        if (data.contains("conversation"))
        {
            ...
            if (conversationData.contains("selfInquiryScope") &&
                conversationData["selfInquiryScope"].is_string())
            {
                outSettings.conversation.selfInquiryScope =
                    conversationData["selfInquiryScope"].get<std::string>();
            }
    ...
    if (outSettings.activeProfile.empty() || outSettings.llm.host.empty() ||
        ...
        (outSettings.conversation.selfInquiryScope != "hard" &&
         outSettings.conversation.selfInquiryScope != "questions") ||
