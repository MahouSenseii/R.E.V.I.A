    ...
    const perceptionSettings& settings, const std::size_t maximum)
{
    ...
    struct Collection
    {
        const perceptionSettings* settings = nullptr;
        std::size_t maximum = 0;
        std::vector<std::string>* titles = nullptr;
    } collection{&settings, maximum, &titles};
    // EnumWindows walks top-level windows in z-order, frontmost first.
    EnumWindows([](const HWND window, const LPARAM data) -> BOOL
    {
        auto* into = reinterpret_cast<Collection*>(data);
        if (into->titles->size() >= into->maximum) return FALSE;
        if (!IsInterestingWindow(window) || IsIconic(window)) return TRUE;
        // The desktop covers every monitor and is not something anyone is doing.
        std::array<wchar_t, 32> windowClass{};
        const int classLength = GetClassNameW(
            window, windowClass.data(), static_cast<int>(windowClass.size()));
        const std::wstring_view className(
            windowClass.data(), static_cast<std::size_t>(std::max(0, classLength)));
        if (className == L"Progman" || className == L"WorkerW") return TRUE;
        // Suspended store apps and hidden shell surfaces report themselves visible.
        BOOL cloaked = FALSE;
        ...
        RECT bounds{};
        if (!GetWindowRect(window, &bounds) || bounds.right - bounds.left < 50 ||
        ...
        if (title.empty() ||
            PerceptionFilter::IsExcludedWindow(*into->settings, ExecutableOf(window), title) ||
            std::find(into->titles->begin(), into->titles->end(), title) != into->titles->end())
        {
            return TRUE;
        }
        into->titles->push_back(title);
        return TRUE;
