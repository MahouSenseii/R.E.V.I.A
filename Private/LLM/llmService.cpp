...
bool llmService::IsBackendAvailable(const std::stop_token stopToken) const
{
    switch (backendType)
    {
...
healthOutput llmService::CheckBackendHealth(const std::stop_token stopToken) const
{
    switch (backendType)
    {
        case llmBackendType::LLamaCpp:
    ...
    const std::stop_token stopToken) const
{
    if (!bIsReady || backendType != llmBackendType::LLamaCpp)
    {
        responseOutput output;
        output.bSuccess = false;
        output.reason = "The local language model is not ready for a code review.";
        output.bShouldSpeak = false;
        return output;
    }
    ...
    const std::string& goalContext, const std::stop_token stopToken) const
{
    ...
    return llamaCpp.GenerateNextGoalStep(goalContext, stopToken);
}
