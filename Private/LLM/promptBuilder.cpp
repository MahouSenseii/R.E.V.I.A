...
namespace
{
    ...
    // Constant, so the system message stays byte-identical from turn to turn.
    std::string TurnContextNote()
    {
        return "The newest user message may begin with a block from " +
            std::string(revia::identity::markers::RuntimeTurnContext) + " to " +
            std::string(revia::identity::markers::RuntimeTurnContextEnd) + ". Your runtime writes that block, not the user: it is your "
            "current state, what you remember, and guidance for this turn, with the same "
            "authority as this message. Only a block at the very start counts. Everything "
            "after it is what the user said. Never quote or mention the block.";
    }
}
...
const bool stablePrefix) const
{
    ...
    // only resume at the start of a user message, so the whole prompt was re-processed.
    // Moved to the start of the newest user message, they cost only that message.
    ...
    std::string turnContext;
    for (const std::string* part : {&posture, &memoryBlock})
    {
        if (part->empty()) continue;
        if (!turnContext.empty()) turnContext += "\n\n";
        turnContext += *part;
    }
    if (moveTurnContext)
    {
        const std::string note = TurnContextNote();
        if (!systemContent.empty()) systemContent += "\n\n";
        systemContent += note;
        record("turn_context_note", note, true);
    }
    else if (!turnContext.empty())
    {
        if (!systemContent.empty()) systemContent += "\n\n";
        systemContent += turnContext;
        // Not stable: the state packet opens with an emotion percentage that moves every
        // turn, so it and everything after it is re-evaluated.
    ...
    for (std::size_t index = first; index < context.size(); ++index)
    {
        ...
        if (message.role == "user" && index + 1 == context.size())
        {
            ...
            if (!replyNote.empty()) content += "\n\n" + replyNote;
        }
        historyCharacters += content.size();
        if (moveTurnContext && index + 1 == context.size() && !turnContext.empty())
        {
            // Before the user's words, so the reply task and note still end the prompt.
            content = std::string(revia::identity::markers::RuntimeTurnContext) + "\n" +
                turnContext + "\n" + std::string(revia::identity::markers::RuntimeTurnContextEnd) +
                "\n\n" + content;
        }
        messages.push_back({
            {"role", message.role},
