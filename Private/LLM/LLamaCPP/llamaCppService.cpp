...
computer: one purposeful PC action. query is a JSON-encoded action object using only supplied approved paths/apps and only an action named in computer_scope.actions, which is the exact set permitted right now and may be shorter than you expect. Files use source (absolute path), copies also destination; windows use application (exe name). If computer_scope.actions offers pointer, key or typing actions, they drive the real desktop: use them only on a target you have actually inspected this session, prefer the smallest action that achieves the point, and never type or click into anything that sends, posts, buys, installs, or deletes. Never invent a target, change settings, or use a shell. If no known target fits, choose another activity.
