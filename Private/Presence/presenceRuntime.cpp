...
void PresenceRuntime::Observe(const runtime::RuntimeEvent& event)
{
    ...
    PresenceSnapshot after;
    {
        ...
        {
            snapshot.attention = revia::utf8::Prefix(event.message, 160);
        }
    ...
    const std::string& audioError)
{
    ...
    {
        response["delivery"] = "voice";
        response["audio_file"] = "";
        response["audio_error"] = audioError;
        if (succeeded && !audio.empty() && audio.size() <= 24U * 1024U * 1024U)
        {
            const auto directory = outboxRoot / "Audio";
            std::error_code error;
            std::filesystem::create_directories(directory, error);
            const auto audioPath = directory / (target.stem().string() + ".wav");
            const auto pending = directory / (target.stem().string() + ".pending");
            bool written = false;
            if (!error)
            {
                std::ofstream file(pending, std::ios::binary | std::ios::trunc);
                file.write(reinterpret_cast<const char*>(audio.data()), static_cast<std::streamsize>(audio.size()));
                file.close();
                written = !file.fail();
            }
            if (written) std::filesystem::rename(pending, audioPath, error);
            if (written && !error)
            {
                publishedAudio = audioPath;
                response["audio_file"] = audioPath.filename().string();
            }
            else
            {
                std::filesystem::remove(pending, error);
                response["audio_error"] = "The Discord voice audio could not be published.";
            }
        }
        if (response["audio_file"] == "" && response["audio_error"] == "")
            response["audio_error"] = "No bounded Discord voice audio was produced.";
    }
    if (!AtomicJsonWrite(target, response))
    {
        std::error_code error;
        if (!publishedAudio.empty()) std::filesystem::remove(publishedAudio, error);
        Notify({"Adapters", "Error", "An adapter reply could not be written."});
        return;
    }
    const bool voiceFailed = request.voiceReply && response.value("audio_file", "").empty();
    Notify({"Adapters", succeeded && !voiceFailed ? "Replied" : "Error",
        voiceFailed ? response.value("audio_error", "Discord voice rendering failed.") :
        (succeeded ? "Wrote a conversation reply for " + request.source + "."
                   : "The " + request.source + " conversation turn failed.")});
}

void PresenceRuntime::PruneVoiceAudio()
{
    // WAVs contain public reply audio only. Expire unconsumed/late replies after
    // ten minutes and keep at most eight files, even if the connector disappears.
    std::error_code error;
    ...
    for (std::filesystem::directory_iterator it(directory, error), end; !error && it != end; it.increment(error))
    {
        ...
        if (!path.filename().string().starts_with("discord-reply-") || path.extension() != ".wav" ||
            !it->is_regular_file(error)) continue;
        const auto time = it->last_write_time(error);
        if (!error) files.emplace_back(time, path);
    }
    std::sort(files.begin(), files.end(), [](const auto& a, const auto& b) { return a.first > b.first; });
    for (std::size_t i = 0; i < files.size(); ++i)
    {
        if (i >= 8 || now - files[i].first > std::chrono::minutes(10))
            std::filesystem::remove(files[i].second, error);
    }
}
...
void PresenceRuntime::RunAdapterInbox(const std::stop_token stopToken)
{
    while (!stopToken.stop_requested())
    {
        ...
        if (now - lastAudioPrune >= std::chrono::seconds(10))
        {
            PruneVoiceAudio();
            lastAudioPrune = now;
        }
    ...
    std::string& outError) const
{
    ...
    try
    {
        ...
        const std::string delivery = document.value("delivery", "text");
        if (delivery != "text" && delivery != "voice")
        {
            outError = "Adapter delivery must be text or voice.";
            return false;
        }
        outEvent.voiceReply = delivery == "voice";
    }
    catch (...)
    {
    ...
    if (outEvent.voiceReply && (outEvent.source != "discord" || outEvent.authorId.empty()))
    {
        outError = "Voice delivery requires Discord and a stable author_id.";
        return false;
    }
    ...
    outEvent.author = revia::utf8::Prefix(outEvent.author, 120);
    return true;
}
    ...
    const int maximumBytes)
{
    ...
    if (error || size < static_cast<std::uintmax_t>(maximumBytes)) return true;
...
void PresenceRuntime::WriteAvatarState(const bool appendEvent)
{
    ...
    bool rotationFailed = false;
    {
        ...
        {
            // Recorded only after the write succeeded, so a failed write does not bar
            ...
            if (appendEvent && !eventsPath.empty())
            {
                ...
                rotationFailed = !RotateAvatarEventsIfNeeded(eventsPath, maximumBytes);
                std::error_code error;
    ...
    // Outside the writer lock. The notice becomes a runtime event that presence itself
    // observes, and an observation that changes the avatar writes state again: holding
    // writerMutex here let a failed write wait on its own lock forever.
    if (writeFailed)
    {
