    ...
    const llm::PrivateMemoryAccess memoryAccess) const
{
    ...
    // keeping raw chunks private; irreversible delivery happens once, below.
    const messageRouter::DeltaHandler bufferOnly = onDelta
        ? messageRouter::DeltaHandler([](const std::string&) {})
        : messageRouter::DeltaHandler{};
    responseOutput output =
        router.RouteMessage(input, context, stopToken, bufferOnly, decision, memoryAccess);
    output.bWasStreamed = false;
    if (output.bSuccess)
    {
        ...
        output.bHardFilterBlocked = firstHard.blocked;
        ...
        if (filterSettings.bAiReviewEnabled && !stopToken.stop_requested())
        {
            ...
            output.bHardFilterBlocked = output.bHardFilterBlocked || finalHard.blocked;
        }
        else
        {
        ...
        if (onDelta && !stopToken.stop_requested())
        {
            onDelta(output.response);
            output.bWasStreamed = true;
        }
    }
    return output;
}
