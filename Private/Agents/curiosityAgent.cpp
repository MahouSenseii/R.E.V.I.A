...
std::vector<std::string> CuriosityAgent::AvailableActions(const IdleActivityContext& idle)
{
    std::vector<std::string> actions = {"silence", "think", "create"};
    if (idle.speakAllowed) actions.emplace_back("speak");
    if (idle.researchAllowed) actions.emplace_back("research");
    if (idle.observationAllowed) actions.emplace_back("observe");
    if (idle.computerAllowed) actions.emplace_back("computer");
    return actions;
}
    ...
    const IdleActivityContext& idle) const
{
    ...
    const std::vector<std::string> available = AvailableActions(idle);
    const responseOutput response = router.GenerateCuriosityPlan(
        BuildContextPrompt(recentConversation, affect, desktopContext, idle),
        available,
        stopToken);
    ...
    CuriosityDecision decision = ParseDecision(response.response);
    // The schema already confines the reply; this holds when a backend ignores it.
    if (decision.valid && std::find(available.begin(), available.end(),
            ToString(decision.action)) == available.end())
    {
        return Invalid("The planner chose " + ToString(decision.action) +
    ...
    const IdleActivityContext& idle)
{
         ...
         ++message)
    {
        ...
        {
            revia::utf8::Truncate(content, limit);
        }
    ...
    json prompt = {
        ...
        {"idle_activity", {
            ...
            {"user_at_computer", idle.userAtComputer},
            {"user_in_full_screen_app", idle.userInFullScreen},
            {"available_actions", AvailableActions(idle)},
            {"unanswered_openings", std::max(0, idle.unansweredOpenings)},
            {"recent_activities", BoundedLine(idle.recentActivities, 800)},
            {"set_aside_recently", BoundedLine(idle.setAside, 600)},
