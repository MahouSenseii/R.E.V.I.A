    ...
    std::string& outReason)
{
    ...
    if (!revia::utf8::IsValid(content))
    {
        outReason = "Malformed UTF-8 was not archived.";
        return false;
    }
    if (const SensitiveFinding finding = DetectSensitiveContent(content))
    {
        ...
        // The rule's category is named (never the matched text) so a false positive can
        // be told apart from a real secret without reopening the turn.
        ++counters.withheldSensitive;
        outReason = "The turn matched a sensitive-content marker (" +
...
std::size_t ConversationArchive::Forget()
{
    ...
    // The write-ahead log is a second file. It still holds every frame written since the
    // last checkpoint -- the forgotten turns among them -- and a checkpoint alone copies
