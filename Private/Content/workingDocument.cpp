...
std::vector<Block>::iterator WorkingDocument::Locate(const std::string& reference)
{
    ...
    // Nine digits always fit in an int. A longer number names no block, and std::stoi
    // throws on it rather than saying so -- "/revise 99999999999" ended the CLI.
    if (!numeric || wanted.size() > 9)
    {
        return blocks.end();
    }
