...
void QwenTtsPool::RequestShutdown()
{
    ...
    condition.notify_all();
    // Closed, not just cancelled. Cancelling kills the worker to abort what it is doing
    // and then lets it be started again; here that meant the phrase it aborted, or the
    // next one queued behind it, relaunched the worker and waited out a full model load
    // before shutdown could finish. Configure builds fresh clients, so nothing reopens
    // these.
    for (QwenTtsClient* client : clients) client->Shutdown();
    if (const std::shared_ptr<QwenTtsClient> design = DesignClient())
    {
        design->Shutdown();
    }
    ...
    const bool latencyCritical)
{
    ...
    const auto predicted = [characters](const VoiceWorkerState& worker)
    {
        return worker.fixedOverheadMilliseconds + worker.millisecondsPerCharacter *
            static_cast<double>(std::max<std::size_t>(1, characters));
    };
    if (latencyCritical && !workers.empty() && !workers.front().busy)
    {
        // Until measured, the planner's placement is the only evidence. Once every idle
        // card has real samples, the measured one wins: the first phrase went to the
        // RTX 5070 on every turn even while it ran at a median real-time factor of 1.18
        // (it also carries chat, self-inquiry and screen vision) and the idle RTX 2070
        // measured 0.61. A clear margin is required so the choice does not flap.
        constexpr std::size_t MinimumSamples = 2;
        constexpr double RequiredAdvantage = 0.85;
        std::size_t fastest = 0;
        bool measured = workers.front().completed >= MinimumSamples;
        for (std::size_t index = 1; index < workers.size() && measured; ++index)
        {
            const VoiceWorkerState& worker = workers[index];
            if (worker.busy) continue;
            if (worker.completed < MinimumSamples) { measured = false; break; }
            if (predicted(worker) < predicted(workers[fastest])) fastest = index;
        }
        if (!measured || fastest == 0 ||
            predicted(workers[fastest]) >= predicted(workers.front()) * RequiredAdvantage)
        {
            return 0;
        }
        return fastest;
    ...
    while (!shuttingDown)
    {
        ...
        for (const Worker& worker : workers)
        {
            states.push_back({worker.busy, worker.fixedOverheadMilliseconds,
                worker.millisecondsPerCharacter, worker.completed});
        }
