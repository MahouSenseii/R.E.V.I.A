    ...
    const std::size_t expectedClips)
{
    ...
    for (const std::size_t size : sizes)
    {
        ...
        // Checked before adding: two lengths near 2^64 wrap to a sum that can equal the
        // payload exactly, and the split would then read far past the end of the body.
        if (size > payloadBytes - total)
        {
            return std::nullopt;
        }
        total += size;
...
{
    if (closed.load() || cancelling.load())
    {
    ...
    while (std::chrono::steady_clock::now() < deadline)
    {
        ...
        if (!running || closed.load() || cancelling.load())
        {
