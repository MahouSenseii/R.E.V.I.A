    ...
    std::string& outError)
{
    ...
    if (processHandle != nullptr)
    {
        ...
        // The worker exited but its handle did not go with it.
        //
        // Returning IsRunning() here meant a dead process left a non-null handle that
        // failed every later Start(), so the service could never come back -- which
        // defeats the recovery this class exists to provide, since the failure it is
        // meant to survive is a fatal CUDA error that kills the worker outright.
        // Releasing the stale handle and continuing is the pattern the Whisper worker
        // already uses.
        Stop();
    }
