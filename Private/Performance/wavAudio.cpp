...
// The largest real format chunk is WAVE_FORMAT_EXTENSIBLE's 40 bytes. Anything far past
// that is a damaged header, and reading it would allocate whatever it claims.
constexpr std::uint32_t MaximumFormatChunkBytes = 1024;
...
std::int16_t Saturate(const double value)
{
    ...
    // A NaN float sample survives clamp and has no integer value; silence is the only
    ...
    const std::int64_t maximumDurationMs)
{
    ...
    while (true)
    {
        ...
        if (std::memcmp(chunk, "data", 4) == 0)
        {
            ...
            // come back to correct it. What the file really holds bounds the length check
            // and the allocation alike; the declared size alone refused a short song as
            // too long, and with no limit asked for gigabytes.
            std::uint64_t payloadBytes = chunkSize;
            const std::streamoff position = file.tellg();
            if (fileBytes > 0 && position >= 0 &&
                static_cast<std::uintmax_t>(position) <= fileBytes)
            {
                payloadBytes = std::min<std::uint64_t>(
                    payloadBytes, fileBytes - static_cast<std::uintmax_t>(position));
            }
            std::int64_t frames = static_cast<std::int64_t>(payloadBytes / frameBytes);
        ...
        // Widened first: 0xFFFFFFFF plus its pad byte wraps to zero in 32 bits.
