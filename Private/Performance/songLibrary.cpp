...
{
    return Lower(revia::actions::PathToUtf8(path.extension())) == ".wav";
}
...
std::string BoundedText(const json& data, const char* key, const std::size_t limit)
{
    ...
    // On a character boundary: titles beyond ASCII are songs like any other, and a
    // split character is invalid UTF-8 wherever the title is serialized.
    revia::utf8::Truncate(value, limit);
...
// Removes `prefix` from the front of both strings when `lower` starts with it. The two
// are kept the same length -- ASCII lowering does not change byte counts -- so the
// original casing survives into the song name.
bool StripPrefix(std::string& lower, std::string& original, const std::string& prefix)
{
...
bool StripSuffix(std::string& lower, std::string& original, const std::string& suffix)
{
    ...
    original.resize(original.size() - suffix.size());
    return true;
}

void TrimBoth(std::string& lower, std::string& original)
{
    while (!lower.empty() && IsTrimmable(lower.front()))
    {
        lower.erase(0, 1);
        original.erase(0, 1);
    }
    while (!lower.empty() && IsTrimmable(lower.back()))
    {
        lower.pop_back();
        original.pop_back();
    }
...
std::optional<SingRequest> ParseSingRequest(const std::string& text)
{
    if (text.empty() || text.size() > 200 || text.front() == '/')
    {
        return std::nullopt;
    }
    std::string original = text;
    std::string lower = Lower(text);
    TrimBoth(lower, original);

    // Her name first, however it was said.
    for (const char* address : {"hey revia", "ok revia", "okay revia", "revia"})
    {
        if (StripPrefix(lower, original, address))
        {
            TrimBoth(lower, original);
            break;
        }
    }
    // Then the politeness around the verb, in any combination.
    bool stripped = true;
    while (stripped)
    {
        stripped = false;
        for (const char* polite : {"please ", "can you ", "could you ", "would you ",
                 "will you ", "go ahead and ", "now "})
        {
            if (StripPrefix(lower, original, polite))
            {
                stripped = true;
            }
        }
    }
    if (!StripPrefix(lower, original, "sing "))
    {
        return std::nullopt;
    }
    TrimBoth(lower, original);
    for (const char* filler : {"me ", "us ", "for me ", "for us ", "the song ", "your "})
    {
        if (StripPrefix(lower, original, filler)) TrimBoth(lower, original);
    }
    for (const char* tail : {" please", " for me", " for us", " now", " again"})
    {
        if (StripSuffix(lower, original, tail)) TrimBoth(lower, original);
    }
    if (lower.empty())
    {
        return std::nullopt;
    ...
    SingRequest request;
    for (const char* any : {"something", "a song", "anything", "one", "a little",
             "something for me", "any song", "one of your songs"})
    {
        if (lower == any)
        {
            request.anySong = true;
            return request;
        }
    }
...
{
    if (songId.empty() || songId.size() > 128 || songId == "." || songId == "..")
    {
        return false;
    }
    // Windows drops a trailing dot or space from a path, so "..." or "song." would name
    // a different folder -- "..." the library itself -- than the one that was checked.
    if (songId.back() == '.' || songId.back() == ' ' || songId.front() == ' ')
    {
        return false;
    }
    // Bytes of 0x80 and above are the pieces of a UTF-8 character, so "Café" and a
    // Japanese title are names like any other. Everything that could leave the folder --
    // separators, a drive colon, control characters -- is still ASCII, and still refused.
    return std::all_of(songId.begin(), songId.end(), [](const unsigned char character)
    {
        return character >= 0x80 || std::isalnum(character) || character == '-' ||
...
{
    SongSummary summary;
    // UTF-8, not string(): on Windows that converts through the ANSI code page and throws
    // on a character the code page lacks, which took the whole listing down with it.
    summary.id = revia::actions::PathToUtf8(entry.path().filename());
    ...
    std::string& outError) const
{
    ...
    std::error_code error;
    const std::filesystem::path directory = root / revia::actions::Utf8ToPath(songId);
