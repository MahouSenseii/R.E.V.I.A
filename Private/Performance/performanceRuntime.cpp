...
{
    MixedSong mixed;
    const std::int64_t maximumDurationMs = config.maximumSongSeconds > 0
        ? static_cast<std::int64_t>(config.maximumSongSeconds) * 1000 : 0;

    PcmAudio instrumental;
    PcmAudio vocal;
    if (asset.hasInstrumental &&
        !ReadWavFile(asset.instrumentalPath, instrumental, mixed.error, maximumDurationMs))
    {
        return mixed;
    }
    if (asset.hasVocal && !ReadWavFile(asset.vocalPath, vocal, mixed.error, maximumDurationMs))
    {
        return mixed;
    }
...
void PerformanceRuntime::Stop(const std::string& reason)
{
    ...
    {
        std::lock_guard lock(mutex);
        // Checked under the same lock the playback thread holds when it goes idle. A stop
        // arriving after the song ended used to mark it "stopping", and with no thread
        // left to clear that, /sing status reported it stopping until the next song.
        if (!performing.load(std::memory_order_acquire))
        {
            return;
        }
...
bool PerformanceRuntime::Start(const std::string& songQuery, std::string& outError)
{
    ...
    std::lock_guard starting(workerMutex);
    if (IsPerforming())
    {
        std::lock_guard lock(mutex);
        // A song already told to stop is on its way out -- "sing something else" stops
        // the current one a moment before asking for the next. Waiting out the last few
        // milliseconds of it beats refusing the request that caused them.
        if (status.state != PerformanceState::Stopping)
        {
            outError = "She is already performing " + status.title + ".";
            return false;
        }
    }
    // A previous song's thread may have finished, or be finishing, without anyone
    // joining it.
    JoinWorker();
    ...
    {
        // One critical section from "performing" to the stop source being reachable, so
        // a Stop that lands the moment the song is accepted is never lost.
        std::lock_guard lock(mutex);
        stopReason.clear();
        status = {};
        ...
        currentStop = worker.get_stop_source();
    }
    outError.clear();
    return true;
}
...
void PerformanceRuntime::Perform(std::stop_token stopToken, SongAsset asset)
{
    ...
    const auto submit = [&](const int index) -> bool
    {
        ...
        // A buffer the device refused is never marked done. Left counted as pending, the
        // loop below waited for it forever: the song never ended, and every later /sing
        // was refused as "already performing".
    ...
    while (!stopToken.stop_requested())
    {
        ...
        if (deviceFailed || (!anyPending && nextSample >= totalSamples))
        {
            break;
        }
    ...
    waveOutClose(device);
    if (deviceFailed && !interrupted)
    {
        finish(PerformanceEventKind::SongFailed,
            "The audio device stopped accepting the song.", positionMs, mixed.durationMs);
        return;
    }
#else
