...
std::string RenderEmotionSection(const ReviaStatePacket& packet)
{
    ...
    {
        section << markers::ResponsePosture << "Neutral at 25% intensity, because "
    ...
    else
    {
        ...
        section << markers::ResponsePosture << name << " at "
...
std::string RenderStatePacket(const ReviaStatePacket& packet, const bool includeRuntimeDetails)
{
    ...
    {
        section(std::string(markers::ExperienceDrift) + "you are now " + drift +
    ...
    if (packet.hasRelationship && packet.relationship.interactionCount > 0)
    {
        ...
        relationship << markers::SpeakerRelationship
    ...
    if (!packet.memories.empty())
    {
        ...
        memories << markers::RelevantMemories;
    ...
    runtime << markers::RuntimeSelfKnowledge
            << "; mention it only if asked or directly relevant): the deterministic "
               "hard response filter is always on; "
