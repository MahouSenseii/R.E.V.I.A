...
const std::string& SubjectOf(const Proposal& proposal)
{
    return proposal.subject.empty() ? proposal.evidence : proposal.subject;
}
    ...
    Proposal& outProposal)
{
    ...
    {
        if (!goals::WorthResumingUnprompted(goal, now))
        {
            continue;
        }
    ...
    const AttentionContext& context)
{
    ...
    if (!lastSubject.empty() && SubjectOf(candidate) == lastSubject)
    {
        consideration.verdict = AttentionVerdict::BelowConfidence;
        policy.RecordSuppressed();
        return consideration;
    ...
    lastSubject = SubjectOf(candidate);
...
AttentionVerdict InitiativeController::SpeakingVerdict(AttentionContext context) const
{
    ...
    context.sinceLastInput = std::chrono::hours{1};
...
void InitiativeController::Expire(const std::string& proposalId)
{
    ...
    for (auto& entry : proposals)
    {
        if (entry.first.id == proposalId && entry.second == ProposalOutcome::Pending)
        {
            ...
            committedProposals.erase(proposalId);
            if (lastSubject == SubjectOf(entry.first))
            {
                lastSubject.clear();
            }
            return;
