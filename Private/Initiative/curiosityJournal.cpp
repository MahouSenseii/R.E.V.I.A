...
bool CuriosityJournal::SameTopic(const std::string& left, const std::string& right)
{
    const std::string normalizedLeft = NormalizeTopic(left);
    const std::string normalizedRight = NormalizeTopic(right);
    ...
    constexpr std::string_view Filler[] = {
        "about", "and", "are", "between", "can", "could", "does", "for", "from", "how",
        "into", "its", "might", "not", "should", "that", "the", "their", "this", "versus",
        "what", "when", "where", "which", "why", "will", "with", "would", "you", "your"};
    const auto meaningfulWords = [&Filler](const std::string& normalized)
    {
        ...
        {
            if (word.size() < 3 ||
                std::find(std::begin(Filler), std::end(Filler), word) != std::end(Filler))
            {
                continue;
            }
            // "artifacts" and "artifact" are one word here.
    ...
    const std::set<std::string> leftWords = meaningfulWords(normalizedLeft);
    const std::set<std::string> rightWords = meaningfulWords(normalizedRight);
    const std::size_t shorter = std::min(leftWords.size(), rightWords.size());
    if (shorter < 2)
    {
        return false;
    }
    std::size_t shared = 0;
    for (const std::string& word : leftWords)
    {
        shared += rightWords.count(word);
    }
    ...
    const std::chrono::system_clock::time_point now) const
{
    ...
    {
        return SameTopic(record.topic, topic) &&
