...
std::chrono::milliseconds SinceLastInput()
{
    ...
    {
        return std::chrono::milliseconds(GetTickCount() - lastInput.dwTime);
    }
#endif
    ...
    const std::chrono::steady_clock::time_point now)
{
    ...
    const bool stuck = now - lastPause >= unbroken;
    const bool changed = stuck != neverPauses;
    neverPauses = stuck;
    return changed;
}

bool InputRhythm::NeverPauses() const
{
    std::lock_guard lock(mutex);
    return neverPauses;
}
    ...
    if (foreground != nullptr)
    {
            ...
            GetMonitorInfoW(monitor, &monitorInfo))
        {
            ...
            const int classLength = GetClassNameW(
                foreground, className.data(), static_cast<int>(className.size()));
            const std::wstring windowClass(
                className.data(), static_cast<std::size_t>(std::max(0, classLength)));
            context.foregroundIsFullScreen = LooksFullScreen(
                {windowRect.left, windowRect.top, windowRect.right, windowRect.bottom},
                {monitorInfo.rcMonitor.left, monitorInfo.rcMonitor.top,
                 monitorInfo.rcMonitor.right, monitorInfo.rcMonitor.bottom},
                IsZoomed(foreground) != FALSE && (style & WS_CAPTION) == WS_CAPTION,
                windowClass == L"Progman" || windowClass == L"WorkerW");
        }
    }
#endif
