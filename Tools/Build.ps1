param(
    [switch]$SkipTests,
    # Optimized build (RelWithDebInfo) in build\release. Debug stays the default.
    [switch]$Release
)
...
Repair-ReviaBuildTree -BuildDirectory $buildDirectory
...
try {
    # Configure and build are retried a few times. build/debug is shared with CLion's
    # "debug" preset, and an IDE reloading the same tree -- or a sync client holding a
    # file for a moment -- fails the step with "failed recompaction: Permission denied"
    # or "Error copying directory". Both steps are idempotent, so a short pause and a
    # second attempt succeeds where the first collided.
    $attempts = 3
    for ($attempt = 1; $attempt -le $attempts; $attempt++) {
        ...
        Write-Warning "CMake configuration failed (attempt $attempt of $attempts); retrying in 5 seconds. If this keeps happening, point CLion's CMake profile at a different build directory."
    ...
    for ($attempt = 1; $attempt -le $attempts; $attempt++) {
        ...
        Write-Warning "Build failed (attempt $attempt of $attempts); retrying in 5 seconds in case a file was briefly locked."
        Start-Sleep -Seconds 5
    }
    ...
    Write-Host "REVIA CLI is ready: $buildDirectory\R_E_V_I_A.exe"
    $desktopPath = Join-Path $buildDirectory 'ReviaDesktop.exe'
