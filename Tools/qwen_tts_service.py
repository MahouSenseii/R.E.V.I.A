...
class QwenRuntime:
    ...
    def trim_cache_if_tight(self, minimum_free_mib: int = 1024) -> bool:
        """Return cached-but-unused CUDA blocks to the card when it is nearly full.

        PyTorch keeps freed activation memory cached for reuse. On the RTX 2070 that grew
        the FP32 worker from about 5.7 GB after load to 7.9 GB after a few replies,
        leaving the card 95% full with nothing actually using the difference -- and no
        room for anything else Revia loads later. Trimming only when the card is short
        keeps the normal path free of any extra allocation cost. Returns whether it
        trimmed.
        """
        ...
        try:
            ...
            if free_bytes // (1024 * 1024) >= minimum_free_mib:
                return False
            # Under the model lock: never while a generation or graph capture runs.
            with self.lock:
                with torch.cuda.device(index):
                    torch.cuda.empty_cache()
            return True
