...
# Wrapped in @(), because PowerShell unwraps a single-element array into the element.
#
# Resolve-Python returns @('py', '-3') when the launcher is present and just the
# executable path when it is not. In the second case the return value came back as a
# bare string, so $pythonCommand[0] indexed the *string* and produced 'C' -- the first
# character of C:\... -- and the installer tried to run a command called C. Anyone with
# Python installed but no py.exe hit it.
