...
The runtime is multi-worker rather than a single blocking pipeline: UI, interactive inference, TTS, STT, presence, and background memory each have independent lifecycle and cancellation ownership. The isolated avatar-state bridge is implemented; importing and rendering a VRM character remains separate presentation work and requires no model or action authority.

## What works now

- Local llama.cpp chat through its OpenAI-compatible chat-completions endpoint.
- A Qt 6 desktop window with horizontal component status and chat controls. The top level stays compact: Chat, Presence, Runtime, Canvas, Permissions, Voice, and Settings. Runtime groups Activity, Pipelines, and Resources in one place; Voice groups profile assignment, creation, and preview without turning each control into another tab.
...
- Opt-in bounded internet grounding. Automatic mode deterministically recognizes current/factual knowledge questions; manual mode leaves ordinary questions local and responds only to explicit web requests. The default visible backend opens a dedicated Edge/Chrome profile, visibly types the DuckDuckGo search, highlights Revia's selected result, visits a bounded number of public HTTPS pages, and returns bounded text plus sources. It never adopts the user's normal browser profile or gives the model a socket, selector, URL bar, or script surface. If that worker cannot start, the fixed DuckDuckGo/Wikipedia API lane remains the startup fallback.
- A persistent affect controller for Revia's own response posture: neutral, curious, pleased, excited, playful, bored, sulky, sad, melancholy, angry, lonely, frustrated, concerned, focused, or confused. The current message is classified before generation, so the feeling shapes the reply being written rather than lagging one turn behind. Negative momentum can deepen or fade across later turns instead of snapping back to pleasant neutrality after every answer. Affect is an internal leaning rather than a tone command: Revia may show it, mask it, exaggerate it, or work through it herself, and the UI and Presence output expose the current state and intensity.
- Layered response filtering at the delivery boundary. The deterministic hard pass is always on and removes control tokens, hidden-prompt leakage, invalid control bytes, oversized output, fabricated runtime state, and emotional coercion after the existing grounding repair. Optional AI response review checks completed candidates while explicitly allowing stubbornness, pouting, boredom, sadness, anger, dislike, smugness, direct teasing, mild insults, playful condescension, harmless pranks, and playing dumb. Negative personality is not itself a filter failure; the remaining character boundaries are targeted hate, credible threats, sustained degrading harassment, coercive dependency, fabricated facts/actions, and sexual framing of the young persona. AI review is a live Settings toggle because it adds a second inference pass; reviewed text cannot reach speech, memory, or conversation history first.
...
- Profile-aware speech on a resource-aware worker pool. Windows SAPI remains the zero-setup fallback; Qwen3-TTS can design a reusable local character voice, clone it for replies, preview it, and assign it independently to each profile. Complete sentence fragments can be generated concurrently on separate GPUs, are played strictly in their original order, and are bounded by both fragment count and buffered WAV bytes. Markdown, code blocks, and raw URLs are removed before speaking, and the UI reports the actual worker, device, dtype, generation, playback, fallback, and timing.
- Hold-to-talk or optional hands-free microphone input using 16 kHz mono PCM, local VAD, and a persistent loopback whisper.cpp `small.en` service. Keeping the model loaded removes per-utterance startup cost; an unavailable service visibly falls back to the CLI path. Automatic transcripts enter the same bounded input arbiter as typed input, while capture and transcription timings remain visible.
- A Presence runtime that publishes atomic avatar state plus an ordered event stream, shows phase/affect/attention/conversation momentum in the UI, and accepts optional conversation-only Discord, stream, or game events through a rate- and size-bounded local inbox. External messages cannot enter command, goal, or application-action routing.
...
- Automatic structured memory selection without requiring a command: durable user identity, preferences, goals, projects, relationships, and constraints are stored in `Memory/revia_memory.db` while transient chat and sensitive credentials are ignored. The completed exchange is now evaluated rather than only the user's half, so a stable like, dislike, relationship judgment, or subject preference that Revia voluntarily expresses can be stored as `self_preference`, `self_relationship`, or `self_opinion`. Passing moods, jokes, vague targets, and unsupported claims about the target are excluded.
...
The editable Qt Designer layout is `Desktop/reviaWindow.ui`; `Desktop/revia.qss` owns the visual theme. Open the layout with the installed Designer:

```powershell
& 'C:\Users\USER\Qt\6.8.3\mingw_64\bin\designer.exe' 'C:\Users\USER\Documents\GitHub\R.E.V.I.A\Desktop\reviaWindow.ui'
```

The `.ui` file owns the window structure and ordinary controls. The focused C++ panels (`PipelinePanel`, `ResourcePanel`, `CanvasPanel`, and `PermissionsPanel`) are inserted into named host layouts from that file so their runtime behavior remains single-purpose. This C++ checkout has no `window.py`; the earlier Python prototype is at `C:\Users\USER\Documents\GitHub\REVIA\revia_controller_py\gui\main_window.py`.
...
Revia's timestamped application log is written beside the executable at `Logs/revia.log`. Runtime → Activity receives the same timestamped lifecycle and timing events plus every component status, including Image, Initiative, Input, Channel, and Language Model events. Warnings and errors have distinct colours and counters, the Runtime tab shows the session's issue count, and the view can filter to warnings/errors or errors only. Runtime → Internet is the focused lookup trace: one row per turn shows the exact query, approved provider, state, source count, and duration; selecting it shows the returned URLs and the bounded grounding text actually supplied to Revia. **Clear** affects only the UI. The application log retains lookup query/source or failure records, and **Open folder** opens the durable application and worker logs. Each chat turn records query embedding, memory retrieval, prompt assembly, request preparation, inference queue wait, wait-to-first-token, decoding after the first token, and total turn time. The `slowest` field identifies the longest non-overlapping stage. Startup, shutdown, automatic-memory classification, document embedding, and memory database saves are timed as well.

Settings exposes the live comfort controls that are useful during normal operation without widening Revia's authority: barge-in, natural conversation starters, their hourly ceiling, AI response review, and resource sampling frequency. The UI makes the always-on hard filter distinct from the optional AI pass and explains the latter's latency cost. Enabling or disabling conversation starters starts or stops the event-driven initiative worker immediately; setting resource sampling to `0` stops that diagnostic worker, while a positive interval restarts it with the new cadence.

Qwen3-VL handles chat/vision; it does not create audio. Voice Studio uses the separate official Qwen3-TTS family: the 1.7B VoiceDesign model turns a natural-language voice description into a reference WAV, then the 0.6B Base model clones that reference for previews and replies. On first run Revia creates `RuntimeData/Voices`, `RuntimeData/Vision`, and `RuntimeData/Capabilities`, then seeds the tracked **Revia Bright** reference and assigns it to the `revia` profile. Existing runtime files and profile choices are never overwritten. Presets and profile assignments are stored in `RuntimeData/Voices/voices.json`, not in the checked-in personality profile, so rebuilding does not erase UI choices. Each authenticated worker binds to its own loopback port beginning at `127.0.0.1:8092`, owns one complete clone model on one selected device, caches extracted clone prompts, and writes diagnostics to a port-suffixed log. Voice design remains one atomic job on the primary worker. The first Create voice and Generate preview operations download their model weights from Qwen's official Hugging Face repositories.

The Resources tab exposes voice placement as **Auto**, **CPU**, or one of the detected CUDA GPUs. The choice persists in `RuntimeData/Preferences/preferences.json` and is applied on the next launch. An explicit GPU choice reserves the configured Qwen3-TTS VRAM budget before llama.cpp fits the chat model; on a shared card that can improve voice latency by moving more chat layers to CPU. If the selected card is missing or cannot meet the minimum free-VRAM budget, the resolved assignment visibly falls back to CPU.

The resource plan resolves `speech.qwenDevices` to exact CUDA indices before the workers start. Each worker checks free memory on its card, reports its real device name and dtype, uses FP16 on the RTX 2070/Turing, and uses BF16 only when the selected device supports it. Clone workers warm after chat is usable; insufficient device memory selects CPU, and a synthesis failure falls back to Windows SAPI. Revia intentionally does not split one conversational sentence across heterogeneous GPUs: each process owns one complete model, while independent sentence jobs are scheduled by measured milliseconds per character and released to playback in order. Independent chat, voice, STT, embedding, perception, and UI pipelines can overlap on their assigned resources.
...
Most agents remain specialized tasks rather than unrestricted autonomous workers. The exception is the bounded curiosity lane: after a real conversation, durable memory, or meaningful affect transition supplies evidence, a cancellable background planner may nominate silence, speech, or one read-only research query. A quiet timer can mature that evidence but cannot invent a topic. Attention policy, cooldowns, hourly limits, cross-restart topic deduplication, and separate browser permission decide whether the nomination can proceed; newer user input cancels or invalidates it. The UI, memory agent, chat, embeddings, speech, and visible browser continue on separate owned pipelines, with interactive conversation taking priority. Long-running goal agents still belong behind a supervisor with cancellation, budgets, health, and the existing action policy.
...
Internet access is disabled by default. Enabling it requires an explicit confirmation because the question text may leave the machine. Automatic lookup can be disabled separately. `/internet` reports the mode, `/internet on|manual|off` changes it, and `/web "query"` always requests one lookup when access is enabled. With `internet.visibleBrowser` enabled, Revia starts the project-owned `Tools/Browser/browserHost.mjs` worker on authenticated loopback port 8095. The worker uses installed Node.js and a dedicated profile at `RuntimeData/Browser/Profile`, finds Edge or Chrome itself, denies downloads and non-GET/HEAD traffic, rejects local/private/non-web targets (including DNS resolutions to private addresses), and shuts down its browser descendants with Revia. Results remain untrusted reference data; Runtime → Internet exposes the exact query, source URLs, and bounded grounding preview, while durable logs retain enough detail to reconstruct failures.
