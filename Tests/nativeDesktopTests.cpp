...
public:
        ...
        const std::chrono::milliseconds limit)
    {
        ...
        while (std::chrono::steady_clock::now() < deadline)
        {
            if (CountOf(needle) >= expected) return true;
...
// A small separately callable fixture suite: no application other than the disposable
// fixture is ever an input target. Approval callbacks change the live native control.
void RunApprovalBindingNativeTests()
{
#ifdef _WIN32
    Fixture fixture;
    Check(fixture.Started(), "Approval fixture could not start.");
    const HWND main = fixture.Window(L"Revia Fixture - Main");
    const HWND button = GetDlgItem(main, 1006);
    Check(main && button, "Approval fixture Send control is missing.");
    SetForegroundWindow(main);
    RECT bounds{};
    GetWindowRect(button, &bounds);
    ActionRequest click;
    click.type = ActionType::ClickPointer;
    click.application = FixtureExecutable;
    click.windowTitle = "Revia Fixture - Main";
    click.input.hasPoint = true;
    click.input.x = (bounds.left + bounds.right) / 2;
    click.input.y = (bounds.top + bounds.bottom) / 2;
    auto guard = std::make_shared<revia::policy::DesktopInputGuard>();
    auto approvals = std::make_shared<revia::policy::DesktopApprovalGate>();
    bool asked = false;
    approvals->SetHandler([&](const auto&) {
        asked = true;
        Check(SetWindowTextW(button, L"Delete everything") != FALSE,
            "Could not relabel the fixture control during approval.");
        return true;
    });
    ...
    Check(SetWindowTextW(button, L"Send") != FALSE, "Could not restore Send.");
    ...
    Check(SUCCEEDED(CoCreateInstance(CLSID_CUIAutomation, nullptr, CLSCTX_INPROC_SERVER,
        ...
        "UI Automation is unavailable for moved-control regression.");
    IUIAutomationElement* element = nullptr;
    Check(SUCCEEDED(automation->ElementFromHandle(button, &element)) && element,
        "Could not observe the fixture Send control.");
    click.resolution.kind = TargetResolutionKind::UiaElement;
    click.resolution.resolvedRuntimeId = ElementRuntimeId(element);
    click.resolution.resolvedAutomationId = WideToUtf8(ElementAutomationId(element));
    click.resolution.resolvedName = WideToUtf8(ElementName(element));
    click.resolution.resolvedControlType = UIA_ButtonControlTypeId;
    element->Release();
    automation->Release();
    if (initialized == S_OK || initialized == S_FALSE) CoUninitialize();
    approvals->SetHandler([&](const auto&) {
        // Exceeds the old observation freshness limit; a new observation must be used.
        std::this_thread::sleep_for(std::chrono::milliseconds(2100));
        Check(SetWindowPos(button, nullptr, 180, 238, 0, 0,
            SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE) != FALSE,
            "Could not move the approved fixture button.");
        return true;
    });
    const auto moved = executor.Execute(click, {});
    Check(moved.succeeded && fixture.WaitForLog("EFFECT sent", std::chrono::seconds(2)),
        "A moved element was not clicked at its fresh bounds after delayed approval: " + moved.message);
    std::cout << "  PASS moved element reacquired after delayed approval\n";

    using namespace revia::computer;
    PayloadVault vault;
    ContentGate gate(vault);
    TaskContent task;
    task.requirement = ContentRequirement::ExactUserContent;
    task.value = "approved body";
    task.submissionRequested = true;
    task.submissionVerb = "send";
    gate.BeginTask(task);
    revia::goals::GoalStep entry;
    entry.action = TypeInto(task.value, L"Revia Fixture - Main");
    entry.action.type = ActionType::SetControlText;
    ComputerTaskContext context;
    const auto draftBefore = ObserveDraft(entry.action);
    Check(gate.Apply(entry, context).allowed, "Fixture draft could not be prepared.");
    const HWND body = GetDlgItem(main, 1002);
    const HWND recipient = GetDlgItem(main, 1003);
    Check(SendMessageW(body, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(L"approved body")) != FALSE, "Could not place fixture draft.");
    revia::goals::StepAttempt attempt;
    attempt.executed = true;
    attempt.outcome = revia::goals::VerificationOutcome::Verified;
    entry.attempts.push_back(attempt);
    revia::goals::Goal goal;
    goal.steps.push_back(entry);
    gate.ObserveGoal(goal);
    Check(gate.SubmissionReady(), "The real fixture draft was not freshly verifiable: " +
        CompareDraftSnapshots(draftBefore, ObserveDraft(entry.action), task.value));
    revia::goals::GoalStep submit;
    submit.action.type = ActionType::InvokeControl;
    submit.action.application = FixtureExecutable;
    submit.action.windowTitle = "Revia Fixture - Main";
    submit.action.control = "1006";
    Check(gate.Apply(submit, context).allowed && submit.action.beforeCommit,
        "A fixture submission was not bound to its draft.");
    WindowsAutomationExecutor invoke(PermissiveSettings(), approvals);
    const auto sentBefore = fixture.CountOf("EFFECT sent");
    approvals->SetHandler([&](const auto&) {
        Check(SendMessageW(body, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(L"changed during approval")) != FALSE,
            "Could not mutate the fixture body.");
        return true;
    });
    const auto changedBody = invoke.Execute(submit.action, {});
    Check(!changedBody.succeeded && fixture.CountOf("EFFECT sent") == sentBefore,
        "A body changed during approval reached Invoke: " + changedBody.message);
    Check(submit.action.submissionStarted && !submit.action.submissionStarted->load(),
        "A pre-injection draft refusal was recorded as starting submission.");
    SendMessageW(body, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(L"approved body"));
    approvals->SetHandler([&](const auto&) {
        Check(SendMessageW(recipient, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(L"another recipient")) != FALSE,
            "Could not mutate the fixture recipient.");
        return true;
    });
    const auto changedRecipient = invoke.Execute(submit.action, {});
    Check(!changedRecipient.succeeded && fixture.CountOf("EFFECT sent") == sentBefore,
        "A recipient changed during approval reached Invoke: " + changedRecipient.message);
    SendMessageW(recipient, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(L""));
    const HWND recipientLabel = GetDlgItem(main, 1020);
    const HWND recipientAddress = GetDlgItem(main, 1021);
    Check(recipientLabel && recipientAddress, "Display-only recipient fixtures are missing.");
    approvals->SetHandler([&](const auto&) {
        SendMessageW(recipientLabel, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(L"Recipient: Bob"));
        return true;
    });
    const auto changedLabel = invoke.Execute(submit.action, {});
    Check(!changedLabel.succeeded && fixture.CountOf("EFFECT sent") == sentBefore,
        "A changed static recipient label reached Invoke: " + changedLabel.message);
    SendMessageW(recipientLabel, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(L"Recipient: Alice"));
    approvals->SetHandler([&](const auto&) {
        SendMessageW(recipientAddress, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(L"bob@example.invalid"));
        return true;
    });
    const auto changedAddress = invoke.Execute(submit.action, {});
    Check(!changedAddress.succeeded && fixture.CountOf("EFFECT sent") == sentBefore,
        "A changed read-only recipient field reached Invoke: " + changedAddress.message);
    SendMessageW(recipientAddress, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(L"alice@example.invalid"));
    approvals->SetHandler([](const auto&) { return true; });
    Check(invoke.Execute(submit.action, {}).succeeded,
        "An unchanged real draft was refused at the submission boundary.");
    Check(fixture.WaitForCount("EFFECT sent", sentBefore + 1, std::chrono::seconds(2)) &&
            fixture.CountOf("EFFECT sent") == sentBefore + 1,
        "The positive submission did not reach the disposable fixture exactly once.");
    Check(submit.action.submissionStarted->load(),
        "The actual submission did not produce its runtime receipt.");
    ContentGate navigationGate(vault);
    task.submissionRequested = false;
    navigationGate.BeginTask(task);
    Check(SendMessageW(button, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(L"Inbox")) != FALSE,
        "Could not prepare the native navigation fixture.");
    ComputerTaskContext navigationContext;
    navigationContext.observation.screen.succeeded = true;
    navigationContext.observation.screen.foregroundTitle = "Revia Fixture - Main";
    ObservedCandidate inbox;
    inbox.id = "1006";
    inbox.name = "Inbox";
    inbox.role = "button";
    inbox.mayInvoke = true;
    navigationContext.observation.candidates.push_back(inbox);
    revia::goals::GoalStep navigate;
    navigate.action.type = ActionType::InvokeControl;
    navigate.action.application = FixtureExecutable;
    navigate.action.windowTitle = "Revia Fixture - Main";
    navigate.action.control = "1006";
    Check(navigationGate.Apply(navigate, navigationContext).allowed,
        "Ordinary navigation before placement was refused.");
    SendMessageW(button, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(L"Send"));
    const auto changedNavigation = invoke.Execute(navigate.action, {});
    Check(!changedNavigation.succeeded && fixture.CountOf("EFFECT sent") == sentBefore + 1,
        "A navigation control changed into Send before execution and bypassed the draft guard.");
    GetWindowRect(button, &bounds);
    navigate.action.type = ActionType::ClickPointer;
    navigate.action.input.hasPoint = true;
    navigate.action.input.x = (bounds.left + bounds.right) / 2;
    navigate.action.input.y = (bounds.top + bounds.bottom) / 2;
    const auto changedNavigationClick = executor.Execute(navigate.action, {});
    Check(!changedNavigationClick.succeeded && fixture.CountOf("EFFECT sent") == sentBefore + 1,
        "The pointer route bypassed the stale navigation constraint.");
    std::cout << "  PASS planning-to-execution navigation drift refused by UIA and pointer\n";
    std::cout << "  PASS body, writable/read-only/static recipient drift refused; unchanged draft submitted once\n";
#endif
}
