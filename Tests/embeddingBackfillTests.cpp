...
public:
    Backend()
    {
        ...
        embeddings.Post("/v1/embeddings", [this](const auto& request, auto& response)
        {
            ...
            // A query embedding is part of classifying a turn -- the lookup of related
            // memories -- not vector work of its own. Answered at once and counted apart,
            // so the ordering checks below still see only turn, learning, and backfill.
            if (input.rfind("search_query: ", 0) == 0)
            {
                ++queryRequests;
    ...
    std::atomic<int> requests{0}, healthRequests{0}, queryRequests{0};
private:
...
void TestCancellationAndInflightDeduplication()
{
    ...
    agent.Submit(router, "Hello", "", revia::agents::ResponseProvenance::NormalGeneration, 501);
...
void TestActualWorkerPriorityAndProgress()
{
    ...
    for (int index = 0; index < 16; ++index)
        agent.Submit(router, "I maintain a long-term astronomy project number " +
            std::to_string(index), "", revia::agents::ResponseProvenance::NormalGeneration, index + 1);
