...
// Valid JSON with a field of the wrong type is a controlled failure, not a throw.
//
// nlohmann throws on a type mismatch, so {"schemaVersion":"2"} -- a string where a
// number belongs -- escaped Load entirely and took whatever had already been decoded
// with it. A file Revia cannot read is a reason to say so and leave the file alone.
void TestAMistypedFieldFailsWithoutThrowingOrLosingTheFile()
{
    ScopedTestDirectory temporary;
    const std::filesystem::path path = temporary.root / "identity.json";
    const std::vector<std::string> mistyped = {
        R"({"schemaVersion":"2"})",
        R"({"schemaVersion":2,"development":{"base":"not-an-object"}})",
        R"({"schemaVersion":2,"mood":{"valence":"warm"}})",
        R"({"schemaVersion":2,"relationships":[{"interactionCount":"many"}]})"
    };
    // The reported case specifically: a schema version that is a string. This one must
    // be refused, because a version that cannot be compared is a file whose shape is
    // unknown -- reading on from there would be guessing.
    {
        const std::string contents = R"({"schemaVersion":"2"})";
        std::ofstream file(path, std::ios::trunc);
        file << contents;
        file.close();
        IdentitySnapshot snapshot;
        std::string error;
        Check(!IdentityStore(path).Load(snapshot, error),
            "An unreadable schema version was accepted.");
        Check(!error.empty(), "The refusal gave no reason.");
    }
    for (const std::string& contents : mistyped)
    {
        {
            std::ofstream file(path, std::ios::trunc);
            file << contents;
        ...
        try
        {
            loaded = IdentityStore(path).Load(snapshot, error);
        }
        catch (...)
        {
            threw = true;
        }
        // Never throwing is the requirement. Whether a particular mistyped field is
        // refused outright or falls back to its default is a judgement per field --
        // an unreadable optional sub-section is reasonably survivable -- but either
        // way it has to be an answer rather than an exception, and a refusal has to
        // say why.
        Check(!threw, "Loading threw instead of returning a failure: " + contents);
        Check(loaded || !error.empty(),
            "A refused load gave no reason: " + contents);
        Check(!loaded || error.empty(),
            "A successful load reported an error anyway: " + contents);

        // The file is what it was. Refusing to read it must never rewrite it.
        std::ifstream check(path);
        const std::string after(
            (std::istreambuf_iterator<char>(check)),
            std::istreambuf_iterator<char>());
        Check(after == contents,
            "Refusing to load the identity file changed it: " + contents);
    }
}


void TestTextThatIsNotUtf8StillSaves()
{
    // Save is bool-and-reason, like Load. A byte that is not UTF-8 -- 0x82 is a
    // Windows console's e-acute -- used to throw out of the JSON writer instead, and
    // every save after it would throw the same way.
    ScopedTestDirectory temporary;
    const std::filesystem::path path = temporary.root / "identity.json";
    IdentitySnapshot snapshot;
    RelationshipState person;
    person.entityId = "console-user";
    person.displayName = "Jos\x82";
    snapshot.relationships[person.entityId] = person;
    std::string error;
    bool saved = false;
    bool threw = false;
    try
    {
        saved = IdentityStore(path).Save(snapshot, error);
    }
    catch (...)
    {
        threw = true;
    }
    Check(!threw && saved, "Text that is not UTF-8 stopped the identity from saving: " + error);
    IdentitySnapshot loaded;
    Check(IdentityStore(path).Load(loaded, error) && loaded.relationships.count("console-user") == 1,
        "An identity saved with a replaced character did not load back: " + error);
}

void RunIdentityPersistenceTests()
{
    TestTextThatIsNotUtf8StillSaves();
    TestAMistypedFieldFailsWithoutThrowingOrLosingTheFile();
