...
// Once both cards have been measured, the first phrase goes to the one that is actually
// faster -- by a clear margin -- instead of always to worker 0.
void TestTheFirstPhraseFollowsMeasuredSpeed()
{
    std::vector<VoiceWorkerState> workers(2);
    workers[0] = {false, 900.0, 60.0, 5};
    workers[1] = {false, 600.0, 25.0, 5};
    Check(SelectIdleVoiceWorker(workers, 40, true) == 1,
        "The first phrase stayed on a measurably slower worker 0.");

    workers[1] = {false, 600.0, 25.0, 1};
    Check(SelectIdleVoiceWorker(workers, 40, true) == 0,
        "The first phrase left worker 0 on a single unrepeated sample.");

    workers[1] = {false, 850.0, 58.0, 5};
    Check(SelectIdleVoiceWorker(workers, 40, true) == 0,
        "The first phrase moved to a worker that is only marginally faster.");
}
...
void TestAClosedVoiceWorkerIsNeverRelaunched()
{
    speechSettings settings;
    // Nothing listens here and the interpreter does not exist, so any attempt to start
    // the worker fails with its own message rather than the shutdown refusal.
    settings.qwenPort = 65011;
    settings.pythonExecutable = "does-not-exist/python.exe";
    revia::speech::QwenTtsClient client;
    client.Configure(settings);

    client.Shutdown();
    // Shutdown's own sequence: the warmup stop cancels after the pool has closed. The
    // cancel used to end by clearing the flag, and the next phrase reloaded the model.
    client.CancelActiveRequest();
    std::string detail;
    Check(!client.IsAvailable(detail) && detail.find("shutting down") != std::string::npos,
        "A closed voice worker was allowed to start again: " + detail);

    // Configure is what reopens a client, and only Configure.
    client.Configure(settings);
    Check(!client.IsAvailable(detail) && detail.find("shutting down") == std::string::npos,
        "A reconfigured voice worker still refused as shutting down: " + detail);
    client.Shutdown();
}
...
{
    TestAClosedVoiceWorkerIsNeverRelaunched();
    {
        revia::speech::SpeechService service;
        auto& pool = revia::speech::SpeechServiceTestAccess::VoicePool(service);
        pool.Configure(TwoDeviceSettings());
        double waited = 0;
        VoicePoolTestAccess::Acquire(pool, 64, false, waited);
        VoicePoolTestAccess::Acquire(pool, 64, false, waited);
        auto adapter = std::async(std::launch::async, [&]
        {
            double wait = 0;
            return VoicePoolTestAccess::Acquire(pool, 64, false, wait);
        });
        Check(adapter.wait_for(std::chrono::milliseconds(50)) == std::future_status::timeout,
            "Adapter render did not wait for an occupied speech worker.");
        service.RequestVoiceShutdown();
        const bool woke = adapter.wait_for(std::chrono::seconds(1)) == std::future_status::ready;
        pool.RequestShutdown(); // Cleanup even if the tested service method regresses.
        const auto worker = adapter.get();
        VoicePoolTestAccess::Release(pool, 0, 64, 0.0);
        VoicePoolTestAccess::Release(pool, 1, 64, 0.0);
        Check(woke && worker >= 2, "Speech shutdown did not release and refuse a waiting adapter renderer.");
    }
    TestAnIdleWorkerIsNeverPassedOverForABusyOne();
    TestTheBestIdleWorkerWinsWhenBothAreFree();
    TestTheFirstPhraseFollowsMeasuredSpeed();
