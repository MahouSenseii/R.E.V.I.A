...
void TestTheProgramDirectoryDoesNotDependOnArgumentZero()
{
#ifdef _WIN32
    // What argv[0] actually holds on Windows: a bare name when launched through PATH,
    // and ANSI text -- here "Zoë" in code page 1252, which is not UTF-8 -- when the
    // install path is not plain ASCII. The first gave the launch directory, and the
    // second threw on conversion; neither may change the answer.
    for (const char* const argumentZero :
        {"ReviaTests", "C:\\Users\\Zo\xEB\\Revia\\ReviaTests.exe", ""})
    {
        const std::filesystem::path directory =
            revia::core::ProgramDirectory(argumentZero);
        Check(std::filesystem::exists(directory / "ReviaTests.exe"),
            "The program directory was not the directory holding this executable: " +
            directory.string());
    }
#else
    Check(revia::core::ProgramDirectory("/opt/revia/R_E_V_I_A") ==
        std::filesystem::path("/opt/revia"),
        "The program directory was not the directory named by argv[0].");
    Check(revia::core::ProgramDirectory(nullptr).empty() &&
        revia::core::ProgramDirectory("").empty(),
        "A missing argv[0] produced a program directory.");
#endif
}
...
void TestAnEmptyClassNeverIdlesTheWorker()
{
    ...
    (void)MemoryAgent::NextClass(empty, round, hasWork);
...
void TestHistoryIsReadBackAfterRestart()
{
    ...
    // Deliberately changed rather than merely satisfied. This record carries the older
    // category name, and it is restored under the one Assess() writes today, because a
    // restored task whose category nothing recognises guards nothing -- which is how a
    // restart came to forget an open task and raise a duplicate for it later.
    Check(task.category == "conversation_latency",
        "A legacy category was not restored under the name the writer uses now.");
...
void TestARestoredCategoryDoesNotProduceADuplicateTask()
{
    ...
    // Counted across both spellings, so the case cannot pass merely because the
    // restored task is filed under a name the count does not look for.
    const auto voiceTasks = std::count_if(
        ...
        {
            return task.category == "voice" || task.category == "first_audio_latency";
        });
    Check(voiceTasks == 1,
        "A second task was raised for a problem already recorded, or the restored one "
        "was dropped. The creation guards must follow the restored history.");
}
...
void RunAuditFindingsTests()
{
    ...
    TestTheProgramDirectoryDoesNotDependOnArgumentZero();
