...
void TestLongWhitespaceAndIdentifiersKeepAttributionBounded()
{
    const std::string whitespace(12000, '\n');
    const std::string plain = "hello" + whitespace + "world";
    Check(ReadSpeechAttribution(plain).quotes.empty(),
        "Ordinary long whitespace acquired a reported speaker.");
    const std::string reported = "Sam" + whitespace + "said:" + whitespace + "\"Hello.\" Thanks.";
    const auto result = ReadSpeechAttribution(reported);
    Check(result.quotes.size() == 1 && result.quotes.front().speaker == QuotedSpeaker::OtherPerson &&
        reported.substr(result.quotes.front().begin,
            result.quotes.front().end - result.quotes.front().begin) == "\"Hello.\"" &&
        result.userAuthoredText.find("Thanks.") != std::string::npos,
        "Whitespace normalization lost original quote offsets or the speaker.");
    Check(ReadSpeechAttribution(std::string(12000, 'x') + " world").quotes.empty(),
        "A long identifier acquired a reported speaker.");
}
...
{
    TestLongWhitespaceAndIdentifiersKeepAttributionBounded();
