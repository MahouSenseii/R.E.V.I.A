...
public:
    explicit Backend(bool curiosityFixture = false, std::string idleAction = "silence", std::string operation = "")
    {
        ...
        server.Post("/v1/chat/completions", [this, curiosityFixture, idleAction, operation](const auto& request, auto& response)
        {
            ...
            if (contextOverflows > 0)
            {
                --contextOverflows;
                response.status = 400;
                response.set_content(R"({"error":{"message":"request exceeds the available context size","type":"exceed_context_size_error"}})", "application/json");
                return;
            }
            if (failReplies.load())
            {
    ...
    {
        std::lock_guard lock(mutex);
        return tests::RuntimeAuthoredText(last);
    }
    json Request()
    {
        std::lock_guard lock(mutex);
    ...
    void OverflowContexts(unsigned count)
...
void TestPerTurnStateLeavesTheSystemMessageStable()
{
    ...
    std::vector<conversationMessage> history{{"user", "Hello there."}};

    const auto send = [&](const std::string& posture)
    {
        service.SetPosture(posture);
        Check(service.GenerateResponse(history).bSuccess, "Generation failed.");
        return backend.Request();
    };
    const json first = send("Current state: calm.");
    history.push_back({"assistant", "Hi."});
    history.push_back({"user", "How are you?"});
    const json second = send("Current state: excited.");

    // The cache-reuse property: turn-local state never touches the system message.
    const auto& firstMessages = first.at("messages");
    const auto& secondMessages = second.at("messages");
    Check(firstMessages.front().value("role", "") == "system" &&
        firstMessages.front() == secondMessages.front(),
        "Per-turn state changed the system message, so no prompt prefix could be reused.");
    Check(secondMessages.front().value("content", "").find("excited") == std::string::npos,
        "Per-turn state was still written into the system message.");
    const std::string latest = secondMessages.back().value("content", "");
    Check(tests::RuntimeBlockOf(latest) == "Current state: excited." &&
        tests::WithoutRuntimeBlock(latest) == "How are you?",
        "The newest user message did not carry this turn's state ahead of the user's words.");
    Check(secondMessages[1].value("content", "") == "Hello there.",
        "An earlier user message kept a runtime block from a previous turn.");
    for (std::size_t index = 1; index < secondMessages.size(); ++index)
        Check(secondMessages[index].value("role", "") != "system",
            "The request contains a late system message rejected by Qwen's template.");

    // A small context compacts the newest message; the block's markers must survive it.
    settings.contextSize = 2048;
    service.ApplySettings(settings, embeddings, profile);
    history.push_back({"assistant", "Fine."});
    history.push_back({"user", "LONG " + std::string(3000, 'u') + " END"});
    const json compacted = send("Current state: " + std::string(5000, 'p') + " STATE END");
    const std::string newest = compacted.at("messages").back().value("content", "");
    Check(newest.rfind(std::string(identity::markers::RuntimeTurnContext), 0) == 0 &&
        newest.find(std::string(identity::markers::RuntimeTurnContextEnd)) != std::string::npos &&
        tests::WithoutRuntimeBlock(newest).starts_with("LONG ") &&
        tests::WithoutRuntimeBlock(newest).ends_with(" END"),
        "Compacting the newest message cut the boundary between runtime text and the user's.");
    history.resize(3);
    settings.contextSize = 8192;

    // The setting restores the previous layout exactly.
    settings.bStablePromptPrefix = false;
    service.ApplySettings(settings, embeddings, profile);
    const json legacy = send("Current state: excited.");
    const auto& legacyMessages = legacy.at("messages");
    Check(legacyMessages.front().value("content", "") == "You are Revia.\n\nCurrent state: excited." &&
        legacyMessages.back().value("content", "") == "How are you?",
        "stablePromptPrefix=false did not keep per-turn state in the system message.");
}
...
void TestContextCompactionKeepsCurrentState()
{
    ...
    const std::string latest = messages.back().value("content", "");
    Check(tests::RuntimeBlockOf(latest).find("angry about a personal jab") != std::string::npos,
    ...
    Check(messages.back().value("role", "") == "user" &&
        tests::WithoutRuntimeBlock(latest).starts_with("LATEST ") &&
        latest.ends_with("END OF CURRENT TURN"),
        "Context compaction lost the latest user's message boundaries.");
    // Deliberately changed rather than merely satisfied.
    //
    // This used to count characters against 3072, which was the old budget's own
    // arithmetic -- usable tokens times two -- restated as an assertion. A request can
    // satisfy that and still be refused by llama.cpp, because two bytes per token is an
    // average of English and nothing else. The budget is now spent in estimated tokens,
    // so the check is made in the same currency, using the function the runtime uses.
    const std::size_t usableTokens = 1536;  // max(1024, 2048 - 128 - 384)
    std::size_t estimated = 0;
    for (const auto& message : messages)
    {
        estimated += revia::llm::EstimateTokens(message.value("content", "")) +
            revia::llm::ChatTemplateTokensPerMessage;
    }
    Check(estimated <= usableTokens && request.value("cache_prompt", false),
        "The actual request exceeded its context budget (" +
            std::to_string(estimated) + " estimated tokens against " +
            std::to_string(usableTokens) + ") or disabled prefix reuse.");
    ...
    Check(brief.at("max_tokens") == 128 && brief.at("messages").size() <= 5 &&
        tests::WithoutRuntimeBlock(brief.at("messages").back().value("content", "")) ==
            "You need therapy.",
    ...
    Check(greeting.at("max_tokens") == 128 && greeting.at("messages").size() <= 5 &&
        tests::WithoutRuntimeBlock(greeting.at("messages").back().value("content", "")) ==
            "How are you today?",
...
void TestContextBudgetsAndBoundedOverflowRecovery()
{
    ...
    llamaCppService service;
    service.ApplySettings(settings, embeddings, profile);
    const std::vector<conversationMessage> context{{"user",
        "CURRENT " + std::string(12000, '\n') + " END"}};
    Check(service.GenerateResponse(context).bSuccess, "Small context request failed.");
    auto request = backend.Request();
    const auto bytes = [](const json& body)
    {
        std::size_t result = 0;
        for (const auto& message : body.at("messages"))
            result += message.at("content").get_ref<const std::string&>().size();
        return result;
    };
    // Independent byte accounting: whitespace cannot pass for free, and response
    // reservation cannot exceed the actual configured context via a minimum floor.
    Check(bytes(request) + request.at("max_tokens").get<std::size_t>() + 384 <= 1024,
        "The transmitted prompt, generation allowance and reserve exceed the small context.");

    settings.contextSize = 4096;
    settings.maxTokens = 128;
    service.ApplySettings(settings, embeddings, profile);
    backend.OverflowContexts(1);
    std::string delivered;
    Check(service.GenerateResponse(context, {}, [&](const std::string& delta) { delivered += delta; }).bSuccess,
        "One explicit context overflow was not recovered.");
    const auto attempts = backend.Requests();
    Check(attempts.size() == 2 && bytes(attempts[1]) < bytes(attempts[0]),
        "Overflow recovery did not retry exactly once with a smaller prompt.");
    Check(!delivered.empty() && delivered.find("exceed_context") == std::string::npos,
        "Recovery delivered an error body as generated content.");
    backend.OverflowContexts(3);
    delivered.clear();
    Check(!service.GenerateResponse(context, {}, [&](const std::string& delta) { delivered += delta; }).bSuccess &&
        backend.Requests().size() == 2 && delivered.empty(),
        "Repeated context overflow was retried without a bound or emitted speech.");
}
