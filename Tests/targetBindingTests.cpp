...
void TestAReusedRuntimeIdCannotHideChangedMeaning()
{
    const TargetBinding approved = Binding(reinterpret_cast<void*>(0x1), 100, "7.1");
    TargetBinding changed = approved;
    changed.controlName = "Delete everything";
    Check(!CompareBindings(approved, changed).empty(),
        "A changed control name was accepted because its runtime id stayed the same.");
    changed = approved;
    changed.controlType = 50000;
    Check(!CompareBindings(approved, changed).empty(),
        "A changed control type was accepted with the approved runtime id.");
    changed = approved;
    changed.taskOrigin = "another-task";
    Check(!CompareBindings(approved, changed).empty(),
        "A binding from another task was accepted.");
    changed = approved;
    changed.policyVersion = "changed-policy";
    Check(!CompareBindings(approved, changed).empty(),
        "A binding from another policy was accepted.");
}

void TestDraftContextSeparatesAggregateBodyFromRecipient()
{
    DraftContextNode document;
    document.runtimeId = "document";
    document.name = "Conversation with Alice";
    document.hasValue = true;
    document.value = "Alice: draft text";
    document.ancestorOfDraft = true;
    DraftContextNode body;
    body.runtimeId = "body-child";
    body.name = "draft text";
    body.insideDraft = true;
    DraftContextNode recipient;
    recipient.runtimeId = "recipient-label";
    recipient.name = "Alice";
    DraftContextNode readOnlyRecipient;
    readOnlyRecipient.runtimeId = "recipient-address";
    readOnlyRecipient.hasValue = true;
    readOnlyRecipient.value = "alice@example.invalid";
    std::vector<DraftContextNode> nodes{document, body, recipient, readOnlyRecipient};
    const auto original = BuildDraftContext(nodes);
    Check(original.has_value(), "A complete draft context was unreadable.");
    nodes[0].value = "Alice: approved body";
    nodes[1].name = "approved body";
    Check(BuildDraftContext(nodes) == original,
        "Editing the draft invalidated context through its ancestor's aggregate text.");
    nodes[2].name = "Bob";
    Check(BuildDraftContext(nodes) != original,
        "A sibling recipient label was omitted from the draft context.");
    nodes[2].name = "Alice";
    nodes[3].value = "bob@example.invalid";
    Check(BuildDraftContext(nodes) != original,
        "A read-only recipient field was omitted from the draft context.");
    nodes[3].valueKnown = false;
    Check(!BuildDraftContext(nodes).has_value(),
        "Unreadable recipient context was silently accepted.");
}
...
void RunTargetBindingTests()
{
    ...
    TestAReusedRuntimeIdCannotHideChangedMeaning();
    TestDraftContextSeparatesAggregateBodyFromRecipient();
