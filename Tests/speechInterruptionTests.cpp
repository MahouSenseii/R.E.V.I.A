...
void TestBatchingWaitsForAFasterThanRealTimeCard()
{
    ScopedTestDirectory directory;
    SpeechService service;
    SpeechServiceTestAccess::ConfigureWithoutWorkers(service, directory.root);
    Check(SpeechServiceTestAccess::BatchCompanionsWith(service, -1.0) == 2,
        "Follow-on phrases were not batched before any speed was measured.");
    Check(SpeechServiceTestAccess::BatchCompanionsWith(service, 0.6) == 2,
        "Follow-on phrases were not batched on a card faster than real time.");
    // A batch on a card slower than playback is silence until the whole batch is done.
    Check(SpeechServiceTestAccess::BatchCompanionsWith(service, 2.0) == 0,
        "Phrases were batched onto a card generating slower than playback.");
}

void TestInterruptedBatchDoesNotDeadlockTheVoice()
{
    ScopedTestDirectory directory;
    SpeechService service;
    SpeechServiceTestAccess::ConfigureWithoutWorkers(service, directory.root);
    std::string phase;
    // A batch that finishes after its reply was interrupted used to publish "Stopped"
    // while still holding the speech mutex, and Notify takes that mutex: the voice
    // thread waited on itself and every later Speak, Submit, and shutdown froze.
    Check(SpeechServiceTestAccess::DiscardBatchReturns(service, phase),
        "Discarding an interrupted batch deadlocked the speech service.");
    Check(phase == "Stopped", "The discarded batch was not reported.");
}
...
void RunSpeechInterruptionTests()
{
    ...
    TestBatchingWaitsForAFasterThanRealTimeCard();
    TestInterruptedBatchDoesNotDeadlockTheVoice();
