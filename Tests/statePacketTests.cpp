...
void TestAFeelingReachesThePromptWithItsCause()
{
    ...
    // A long cause is bounded on a character, never inside one. The cause reaches the
    // prompt, and a prompt holding a split character cannot be encoded as JSON, which
    // failed every turn until the cause expired. 159 ASCII bytes put the cut through
    // the middle of the two-byte e-acute.
    stimulus.description = std::string(159, 'a') + "\xC3\xA9 and then some more words";
    (void)runtime.Observe(stimulus, {});
    const std::string boundedCause = runtime.Current().cause;
    Check(!boundedCause.empty() && boundedCause.size() <= 160 &&
            revia::utf8::IsValid(boundedCause),
        "A long cause was cut through a character and would break the prompt.");
    runtime.Reset();
