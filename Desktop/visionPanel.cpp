...
void VisionPanel::CaptureCameraFrame()
{
    ...
    captureWorker = std::jthread([this, selection]()
    {
        ...
        // ends the application. A capture that threw is a capture that failed.
        revia::vision::CameraFrame frame;
        try
        {
            frame = session.CaptureCameraFrame(false, selection);
        }
        catch (const std::exception& error)
        {
            frame.succeeded = false;
            frame.reason = std::string("The camera capture failed: ") + error.what();
        }
